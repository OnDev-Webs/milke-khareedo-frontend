...
interface GoogleMapProps {
    ...
    zoom?: number;
    markers?: Array<{ id?: string; lat: number; lng: number; title?: string; label?: string }>;
    className?: string;
}
...
export default function GoogleMapComponent({
    ...
    useEffect(() => {
        if (map && markers.length > 0 && typeof window !== "undefined" && (window as any).google?.maps) {
            ...
            markers.forEach((marker) => {
                if (marker.lat && marker.lng && !isNaN(marker.lat) && !isNaN(marker.lng) && marker.lat !== 0 && marker.lng !== 0) {
            ...
            setTimeout(() => {
                ...
                if (markers.length > 1) {
                    ...
                    map.setZoom(14);
                    console.log(`✅ Map centered on single marker ${markers[0].label || '?'}`);
                }
            }, 100);
        }
    }, [map, markers]);
    ...
    const createCustomMarkerIcon = (labelText: string): google.maps.Icon | undefined => {
        ...
        >
            {markers.map((marker, index) => {
                ...
                if (!markerIcon) {
                    ...
                    console.warn(`Failed to create custom icon for marker ${label}, using default`);
                    return (
                        <Marker
                            key={markerKey}
