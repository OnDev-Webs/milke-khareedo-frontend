...
// components/CitySelector.tsx   (now really an Area selector for Hyderabad)
"use client";
...
// Only Hyderabad areas — no countries, no other cities
export interface Area {
    area: string;
    value: string; // "Area, Hyderabad"
}

const hyderabadAreas: Area[] = [
    { area: "Hyderabad", value: "Hyderabad, Hyderabad" },
    { area: "Hitech City", value: "Hitech City, Hyderabad" },
    { area: "Kondapur", value: "Kondapur, Hyderabad" },
    { area: "Madhapur", value: "Madhapur, Hyderabad" },
    { area: "Kukatpally", value: "Kukatpally, Hyderabad" },
    { area: "Miyapur", value: "Miyapur, Hyderabad" },
    { area: "Nizampet", value: "Nizampet, Hyderabad" },
    { area: "Bachupally", value: "Bachupally, Hyderabad" },
    { area: "Jubilee Hills", value: "Jubilee Hills, Hyderabad" },
    { area: "Banjara Hills", value: "Banjara Hills, Hyderabad" },
    { area: "Manikonda", value: "Manikonda, Hyderabad" },
    { area: "LB Nagar", value: "LB Nagar, Hyderabad" },
    { area: "Uppal", value: "Uppal, Hyderabad" },
    { area: "Dilsukhnagar", value: "Dilsukhnagar, Hyderabad" },
    { area: "Ameerpet", value: "Ameerpet, Hyderabad" },
    { area: "SR Nagar", value: "SR Nagar, Hyderabad" },
    { area: "KPHB Colony", value: "KPHB Colony, Hyderabad" },
    { area: "BHEL", value: "BHEL, Hyderabad" },
    { area: "Shamshabad", value: "Shamshabad, Hyderabad" },
    { area: "Shamirpet", value: "Shamirpet, Hyderabad" },
    { area: "Attapur", value: "Attapur, Hyderabad" },
    { area: "Tolichowki", value: "Tolichowki, Hyderabad" },
    { area: "Mehdipatnam", value: "Mehdipatnam, Hyderabad" },
    { area: "Secunderabad", value: "Secunderabad, Hyderabad" },
    { area: "Begumpet", value: "Begumpet, Hyderabad" },
    // Add more Hyderabad areas here...
];
...
export default function CitySelector({
    ...
    // Set default Hyderabad area
    useEffect(() => {
        if (!selectedArea) {
            const defaultArea =
                hyderabadAreas.find(
                    (a) => a.area.toLowerCase() === "gachibowli" // change default if needed
                ) || hyderabadAreas[0];

            setSelectedArea(defaultArea);
            onChange?.(defaultArea.value);
        }
    ...
    useEffect(() => {
        if (value) {
            const found = hyderabadAreas.find((a) => a.value === value);
    ...
    const filteredAreas = useMemo(() => {
        if (!searchQuery.trim()) return hyderabadAreas;

        const query = searchQuery.toLowerCase().trim();
        return hyderabadAreas.filter(
            (area) =>
                area.area.toLowerCase().includes(query) ||
                area.value.toLowerCase().includes(query)
        );
    }, [searchQuery]);
    ...
    const visibleAreas = useMemo(() => {
        return filteredAreas.slice(visibleRange.start, visibleRange.end);
    }, [filteredAreas, visibleRange]);

    const handleScroll = useCallback(
        (e: React.UIEvent<HTMLDivElement>) => {
            const target = e.currentTarget;
            const scrollBottom = target.scrollHeight - target.scrollTop - target.clientHeight;
            if (scrollBottom < 100 && visibleRange.end < filteredAreas.length) {
                const increment = debouncedSearch.trim() ? 30 : 10;
                setVisibleRange((prev) => ({
                    start: prev.start,
                    end: Math.min(prev.end + increment, filteredAreas.length),
                }));
            }
        },
        [filteredAreas.length, visibleRange.end, debouncedSearch]
    );
    ...
    const handleKeyDown = (e: React.KeyboardEvent, area?: Area) => {
        if (e.key === "Enter" && area) {
            ...
            {isOpen && (
                <>
                    ...
                    <div
                        ...
                        onClick={(e) => e.stopPropagation()}
                        style={{
                            width: dropdownRef.current?.offsetWidth || "240px",
                            minWidth: "240px",
                        }}
                    >
                        {/* Search Input */}
                        <div className="sticky top-0 bg-white p-3 border-b-2 border-gray-100 z-10">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    placeholder="Search area in Hyderabad..."
                                    className="w-full rounded-lg border-2 border-gray-200 bg-gray-50 py-2 pl-10 pr-3 text-sm text-gray-800 placeholder-gray-400 focus:border-[#1C4692] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#1C4692]/20 transition-all"
                                    onKeyDown={(e) => handleKeyDown(e)}
                                    onClick={(e) => e.stopPropagation()}
                                />
                        ...
                        >
                            {filteredAreas.length === 0 ? (
                                <div className="px-4 py-12 text-center">
                                    <p className="text-sm text-gray-500">
                                        No areas found in Hyderabad
                                    </p>
                                </div>
                            ) : (
                                <div className="py-1">
                                    {visibleAreas.map((area, index) => {
                                        const isSelected = selectedArea?.value === area.value;
                                        return (
                                            <button
                                                key={area.value}
                                                type="button"
                                    ...
                                    {visibleRange.end < filteredAreas.length && (
                                        <div className="px-4 py-2 text-center text-xs text-gray-400">
                                            Scroll for more areas ({visibleRange.end} of {filteredAreas.length})
                                        </div>
                                    )}
                                </div>
                            )}
